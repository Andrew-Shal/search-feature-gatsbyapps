...
const SearchV1 = ({
	...
	const { HotelFilterByEnums, LandFilterByEnums, RealEstateFilterByEnums } =
	...
	const handleSeachBtnClicked = (onSearch) => {
		...
		const hasBuildingtypeFilter = searchParams.filters.buildingType
			? true
			: false
		const hasBuildingStatusFilter = searchParams.filters.buildStatus
			? true
			: false
				...
				<S.FilterByGroup>
					{searchByTypeFilterOptions?.buildingStatus.length ? (
						<FilterByDropDown
							...
							options={searchByTypeFilterOptions.buildingStatus}
					...
					{searchByTypeFilterOptions?.buildingStyle.length ? (
						<FilterByDropDown
							...
							options={searchByTypeFilterOptions.buildingStyle}
